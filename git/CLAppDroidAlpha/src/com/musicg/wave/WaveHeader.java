...
public class WaveHeader {
	...
	public static WaveHeader parseString(String s){
		WaveHeader wh=new WaveHeader();
		boolean chunkIdB=false,chunkSizeB=false,formatB=false,
				subChunk1IdB=false,subChunk1SizeB=false,audioFormatB=false,
				channelsB=false,sampleRateB=false,byteRateB=false,
				blockAlignB=false,bitsPerSampleB=false,subChunk2IdB=false,
				subChunk2SizeB=false;
		String index=new String();
		String value=new String();
		int i=0;
		while(i<s.length()){
			index=s.substring(i, i+1);
			if(index.compareTo(":")==0){
				i=i+2;
				index=s.substring(i, i+1);
				while(index.compareTo("\n")!=0){
					value+=index;
					i++;
					index=s.substring(i, i+1);
				}
				...
				if(!chunkIdB){
					...
					wh.setChunkSize(Long.parseLong(value));
					chunkSizeB=true;
				}
				else if(!formatB){
					wh.setFormat(value);
				...
				else if(!subChunk1IdB){
					wh.setSubChunk1Id(value);
					subChunk1IdB=true;
				}
				else if(!subChunk1SizeB){
					wh.setSubChunk1Size(Long.parseLong(value));
					subChunk1SizeB=true;
				}
				else if(!audioFormatB){
					wh.setAudioFormat(Integer.parseInt(value));
					audioFormatB=true;
				}
				else if(!channelsB){
					wh.setChannels(Integer.parseInt(value));
					channelsB=true;
				}
				else if(!sampleRateB){
					wh.setSampleRate(Integer.parseInt(value));
					sampleRateB=true;
				}
				else if(!byteRateB){
					wh.setByteRate(Long.parseLong(value));
					byteRateB=true;
				}
				else if(!blockAlignB){
					wh.setBlockAlign(Integer.parseInt(value));
					blockAlignB=true;
				}
				else if(!bitsPerSampleB){
					wh.setBitsPerSample(Integer.parseInt(value));
					bitsPerSampleB=true;
				}
				else if(!subChunk2IdB){
					wh.setSubChunk2Id(value);
					subChunk2IdB=true;
				}
				else if(!subChunk2SizeB){
					wh.setSubChunk2Size(Long.parseLong(value));
					subChunk2SizeB=true;
				}
				else{
					return wh;
