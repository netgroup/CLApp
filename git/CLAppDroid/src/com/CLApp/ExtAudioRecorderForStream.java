...
public class ExtAudioRecorderForStream 
{
	private final static int[] sampleRates = {44100, 22050, 11025, 8000};
	
	public static ExtAudioRecorderForStream getInstanse(Boolean recordingCompressed)
	{
		ExtAudioRecorderForStream result = null;
		
		if(recordingCompressed)
		{
			result = new ExtAudioRecorderForStream(	false, 
											AudioSource.MIC, 
											sampleRates[3], 
		...
		else
		{
			...
			do
			{
				result = new ExtAudioRecorderForStream(	true, 
												AudioSource.MIC, 
												sampleRates[i], 
												AudioFormat.CHANNEL_IN_MONO,
												AudioFormat.ENCODING_PCM_16BIT);
				
			} while((++i<sampleRates.length) & !(result.getState() == ExtAudioRecorderForStream.State.INITIALIZING));
		}
		return result;
	}
	...
	*/
	 ...
	 * 
	 */ 
	public ExtAudioRecorderForStream(boolean uncompressed, int audioSource, int sampleRate, int channelConfig, int audioFormat)
	{
		try
		{
		...
		} catch (Exception e)
		{
			...
			{
				Log.e(ExtAudioRecorderForStream.class.getName(), e.getMessage());
			}
			else
			{
				Log.e(ExtAudioRecorderForStream.class.getName(), "Unknown error occured while initializing recording");
			}
			state = State.ERROR;
		}
	}
	...
	public void setOutputFile(String argPath)
	{
		...
		catch (Exception e)
		{
			...
			{
				Log.e(ExtAudioRecorderForStream.class.getName(), "Unknown error occured while setting output path");
			}
			state = State.ERROR;
		}
	}
	...
	*/
	public void prepare()
	{
		try
		{
			if (state == State.INITIALIZING)
			{
				if (rUncompressed)
				{
					...
					{
						Log.e(ExtAudioRecorderForStream.class.getName(), "prepare() method called on uninitialized recorder");
						state = State.ERROR;
					}
				}
				else
			...
			{
				Log.e(ExtAudioRecorderForStream.class.getName(), "prepare() method called on illegal state");
				release();
				state = State.ERROR;
			}
		}
		catch(Exception e)
		{
			...
			{
				Log.e(ExtAudioRecorderForStream.class.getName(), "Unknown error occured in prepare()");
			}
			state = State.ERROR;
		}
	}
	...
	public void release()
	{
		...
		else
		{
			if ((state == State.READY) & (rUncompressed))
			{
				...
				{
					Log.e(ExtAudioRecorderForStream.class.getName(), "I/O exception occured while closing output file");
				}
	...
	public void start(ArrayBlockingQueue<Byte> pipeTX,ArrayBlockingQueue<Byte> pipeIN)
	{
		...
		{
			Log.e(ExtAudioRecorderForStream.class.getName(), "start() called on illegal state");
			state = State.ERROR;
		}
	}
	...
	public void stop()
	{
		...
		{
			Log.e(ExtAudioRecorderForStream.class.getName(), "stop() called on illegal state");
			state = State.ERROR;
		}
	}
